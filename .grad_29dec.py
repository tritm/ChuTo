...
#   c = matrix([numpy.random.randint(10,1000) for i in range(L)])
    ...
    def hfunc(m,n,p,k,z):
        u = utility(z)
        if m==p and n==k: 
            return endhost_weight[m]/sum(z[m][j] for j in range(J))**2 +\
    ...
    def func(x):
        ...
        u = utility(z)
        f = -sum(endhost_weight[i]*log(sum(z[i][j] for j in range(J))) for i in range(I)) + q*sum(math.exp(u[l]) for l in range(L))
        return f
    def gradFunc(x):
        z = numpy.reshape(x,(I,J))
        Df = numpy.empty((I,J))
        u = utility(z)
        df1 = lambda m,n: -endhost_weight[m]/sum(z[m][j] for j in range(J)) + \
