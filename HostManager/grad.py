...
class Netcal(object):
    def routing_matrix(self): 
        ...
        H[1][2][0] = 1.
        return H 
    def __init__(self,q=0.001,NULL = 1e-300, I = 2, J = 2, L = 3, capa = 10):
        self.q = q
        self.NULL = NULL
        self.I = I
        self.J = J
        ...
        self.capa = numpy.reshape([capa, capa, capa],(3,1))
...
#         self.q * sum(self.H[m][l][n] * self.H[p][l][k] / (self.capa[l] ** 2) * math.exp(u[l]) for l in range(self.L)) # }}}
    def func(self,x): 
        ...
        u = self.utilization(z)
        f = -sum(self.endhost_weight[i] * log(sum(z[i][j] for j in range(self.J))) for i in range(self.I))  
#             + self.q * sum(math.exp(u[l]) for l in range(self.L))
        return f 
    def gradFunc(self,x): 
        z = numpy.reshape(x, (self.I, self.J))
